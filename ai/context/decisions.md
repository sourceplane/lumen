...
- AWS IAM roles and shared Terraform state buckets are owned by `aws-admin`.
  `lumen` consumes the repo-scoped roles created there.
- Terraform state for `lumen` uses the shared S3 buckets
...
- Supabase target is fixed:
  ...
  - project/database names `lumen-stage` and
    `lumen-prod`
...
- BF5 (saas-bootstrap-factory): infra components publish their consumable
  ...
  write scope (`sourceplane/lumen/*`) already cover it.
- BF4 (saas-bootstrap-factory): the Hyperdrive binding name is `PLATFORM_DB`
  (was `LUMEN_DB`; epic decision D1). Renamed atomically across all 12
...
- Task 0040 CORS policy: `api-edge` allows only the Pages production origin
  (`https://lumen-web-console.pages.dev`), Pages preview subdomains
  (`*.lumen-web-console.pages.dev`), and localhost/127.0.0.1 for dev.
...
- Web-console hosting should be environment-specific after Task 0040: stage and
  ...
  names for Task 0041 are `lumen-web-console-stage` and
  `lumen-web-console-prod`; each deployed console should be locked to its
...
- Verified plan role ARNs from Task 0004:
  - `arn:aws:iam::306024784101:role/dev-github-sourceplane-lumen-plan`
  - `arn:aws:iam::306024784101:role/stage-github-sourceplane-lumen-plan`
  - `arn:aws:iam::306024784101:role/prod-github-sourceplane-lumen-plan`
- Verified deploy role ARNs from Task 0004:
  - `arn:aws:iam::306024784101:role/dev-github-sourceplane-lumen-production-deploy`
  - `arn:aws:iam::306024784101:role/stage-github-sourceplane-lumen-production-deploy`
  - `arn:aws:iam::306024784101:role/prod-github-sourceplane-lumen-production-deploy`
- `aws-admin` PR #26 and main CI run `26204953054` verified Secrets Manager
  lifecycle access for `sourceplane/lumen/*`.
- `lumen` main CI run `26209010693` verified Supabase stage/prod
