...
| [`saas-bootstrap-factory/`](./saas-bootstrap-factory/) | **BF** | Draft (not started) | stack-tectonic, infra/terraform/*, all wrangler surfaces | Make the repo instantiable: kill hardcoded identity/resource IDs (config indirection + deploy-time wiring), complete the infra story (foundation, preflight, domain v5), then author this repo's `blueprint.yaml`. The Blueprint contract + instantiator + upgrade **engine** is the orun binary's (→ `orun/specs/orun-scaffolding/`). Acceptance test: a second live SaaS ("acme") from `instance.yaml` only, via `orun instantiate`. |
...
| [`saas-baseline-tracking/`](./saas-baseline-tracking/) | **BT** (port of the cross-repo epic built in cirrus; over orun BT-O1–O4) | ✅ Ported (BT1–BT5 + CI tests; rehearsal gated) | flows/common, flows/phases/*, flows/agent, flows/testing, tests/flows | The bootstrap tracks itself in the workspace's task plane: epic `infra-baselining`, one milestone per phase, one task per landing, PR branches `orun/BASE-n-<phase>` so every landing binds and folds to done from observed evidence. |
