...
This is the **cross-epic index** for the Lumen SaaS starter. It groups the
...
| **FT** | [`epics/saas-free-tier/`](./epics/saas-free-tier/) | In progress | Run the whole baseline on a Cloudflare account with no Workers Paid subscription, as a profile rather than a fork: FT0 profile foundation (cron topology + scheduled-pass budgets) · FT1 single-environment deployment · FT2 chain-depth/subrequest audit · FT3 request-path cost instrumentation · FT4 connection reuse · FT5 console bundle · FT6 monolith mode (gated). The binding constraint is chained CPU: a service-binding chain shares one 10ms budget on the free plan. |
| **BT** | [`epics/saas-baseline-tracking/`](./epics/saas-baseline-tracking/) | ✅ Ported (rehearsal gated) | The bootstrap as tracked work, ported from cirrus: `track.sh` over `orun task …` · `land-pr.sh` through `orun pr open` · phases ensure milestone + task, the umbrella lays out the epic · phase 01 as PR #1 · the brief lays out the programme and reports the rollup · contract tests in CI (`tests/flows`). |
