...
| `common/` | shared building blocks the phases compose: `ctx.sh` (product resolve/clone), `preflight.sh` (workspace readiness + self-heals), `apply-blueprint.sh` (slice apply + rebrand + slug resolution), `land-pr.sh` / `push-main.sh` (landings), `converge.sh` (watch + auto-resume ×3), `umbrella.sh` (phase checkpoint + resume probes), `redeploy-marker.sh` (force components into the `--changed` set), `verify-endpoints.sh`, `create-secrets.sh` (brokered provider secrets), `render-deployment-docs.sh` (phase 08's renderer) |
