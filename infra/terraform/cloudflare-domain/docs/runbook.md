...
- **Provider auth failure**: the workspace integration connection is
  ...
  from it per run.
- **Plan fails: zone not found**: the product zone does not exist in
  this Cloudflare account yet (zone creation is an account-plan
  operation done in the dashboard) — create it, re-run.
